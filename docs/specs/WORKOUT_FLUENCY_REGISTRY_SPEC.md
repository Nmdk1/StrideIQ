# Workout Fluency Registry — Specification v0.2.17
...
## 7. Schema: required fields per variant (v0.2.17)
...
**v0.2.17 KB pilot delivers:** Threshold variant markdown **SME-approved**; long-family **`long_run_pilot_v1.md`**: **8**× **`approved`**. **Pilot 3** **`easy_pilot_v1.md`**: **6** variant rows — **4**× **`approved`** (`easy`, `recovery`, `easy_strides`, `rest`, founder **2026-03-20**) + **2**× **`draft`** (`hills`, `strides` neuromuscular-touch rows—awaiting founder SME). **§7.0** — KB prose is input to **deterministic** plan construction. **Remaining v1 stems** (VO2 / intervals primary track; **`repetitions`** stem and rich **hill-repeat** progressions still per build sequence / defer) — **not** complete until those pilots exist and SME-**approve** per founder scope **before** Phase 3 wiring.
...
## 10. Acceptance criteria (v0.2.17 doc + KB pilot)
...
## 11. Acceptance criteria (wiring phase — beyond v0.2.17 KB)
...
*End of v0.2.17 — Pilots 1–3 **core** easy KB **approved**; **two** Pilot 3 neuromuscular rows **`draft`**; **VO2** / remaining v1 tracks per build sequence **before** Phase 3 wiring; product voice is StrideIQ synthesis, not third-party bibliography.*
