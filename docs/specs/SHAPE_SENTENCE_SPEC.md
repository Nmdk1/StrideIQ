...
**Current gate (March 8, 2026):** Recent coverage is materially better
than historical coverage, but the corpus is not yet stable enough to
build the product-identity layers below. The coverage fixes in
`docs/BUILDER_INSTRUCTIONS_COVERAGE_FIXES.md` must ship first.
...
**GATED — Do not build yet.** This section is preserved product thinking,
not the next implementation step.

Build gate:
- shape sentence coverage >= 80% across Michael, Larry, and BHL
- no new trust-breaking regressions on the 14-activity verification set
- historical over-segmentation fixes from
  `docs/BUILDER_INSTRUCTIONS_COVERAGE_FIXES.md` shipped first

Current state:
- recent coverage is materially improved
- historical coverage remains too suppressed / unstable
- `title_authorship`, `title_source`, and the identity-model rollout are
  premature until extraction quality is trustworthy enough to carry
  product identity

The shape sentence is not metadata attached to an activity. When it's
right, it IS the identity of the activity. This makes the relationship
between the athlete's original title and the system sentence a core
product architecture decision.

**The rule:** The system sentence leads when the athlete didn't speak.
The athlete's title leads when they did. The chart confirms both.

#### Title authorship detection (three tiers)

The system must determine whether the activity name was authored by the
athlete or auto-generated by the device/platform. This is NOT binary —
there are three confidence tiers:

**Tier 1: Known auto-generated (system sentence leads)**
```
Garmin patterns:
  "{Location} Running"        → "Lauderdale County Running"
  "{Location} Walking"        → "Bay St Louis Walking"
  "{Location} Cycling"        → "Meridian Cycling"

Strava patterns:
  "Morning Run", "Afternoon Run", "Lunch Run", "Evening Run"
  "Morning Walk", "Afternoon Walk"
```
The system sentence replaces these entirely. "7 miles easy" is the
identity. "Lauderdale County Running" disappears.

**Tier 2: Clearly authored (athlete title leads)**
```
Contains workout structure:   "6x400m hill repeats with jog recovery"
Contains personal narrative:  "Stunning weather. No hamstring locking"
Contains route description:   "Flint farm → east common → norton"
Contains race context:        "Father son, state age records 10 mile!"
Contains paces/times:         "2 @8:30, 1@7:30, 1@7:10, 2 easy"
```
The athlete's title is primary. The system sentence appears underneath
as structural confirmation. When they agree ("hill repeats" + "5 miles
with 6 hill repeats"), trust compounds. When they differ, both show
and the chart resolves.

**Tier 3: Ambiguous (default to authored)**
```
...
"Long one"
```
Could be auto-generated by a platform, could be athlete shorthand.
**Default to treating it as authored.** Suppressing a real athlete
title is worse than showing a system sentence under a generated one.
The cost of the wrong default is asymmetric — so err on the side of
preserving athlete voice.

#### How titles display

**Activity list:**
```
When Tier 1 (auto-generated):
    Title:    "7 miles easy with 4 strides"        ← system sentence IS the title
    Subtitle: (none)

When Tier 2 (authored):
    Title:    "Stearns Hill torture chamber"         ← athlete's words
    Subtitle: "5 miles with 6 hill repeats"          ← system confirmation

When Tier 3 (ambiguous):
    Title:    "Track workout"                        ← preserved as authored
    Subtitle: "8 miles with 12x400m at 5:22"         ← system adds structure

When suppressed (uncertain shape):
    Title:    whatever exists (auto or authored)
    Subtitle: (none)
```

**Activity detail page:**
- Athlete title (if authored) at the top
- System sentence below, visually connected to the chart
- The sentence teaches the athlete what to look for in the chart
- The chart proves the sentence is true
- If they disagree, the athlete sees both and draws their own conclusion

#### Strava title fallback for Garmin-sourced athletes

**Problem:** Athletes who sync via Garmin get auto-generated names
("Lauderdale County Running"). Many of these athletes ALSO edit their
titles on Strava after the run. Michael's Strava has authored names
for most activities — but the system only sees the Garmin webhook push
and stores the Garmin auto-name. The athlete's voice is lost.

"Father son, state age records 10 mile!" is the identity of that
activity. "Bay St Louis Running" is noise. The system has the noise.

**Fix:** For athletes connected to both Garmin and Strava, check
Strava for an updated title after Garmin ingestion.

**This must be async, not blocking.** The Garmin webhook fires, the
activity ingests, the shape sentence generates immediately from the
stream data. The Strava title check runs in the background — a
separate lightweight task. If a better title exists on Strava, it
updates `Activity.name`. The athlete never sees a degraded state.
The system gets smarter over time.

This also handles the common case where athletes edit their Strava
title hours or days after the run. The async check can run on a
schedule (e.g., check activities from the last 48 hours for title
updates) or be triggered by Strava webhook events.

```python
@celery_app.task(name="tasks.check_strava_title_update")
def check_strava_title_update(activity_id: str):
    """Async check for updated Strava title on Garmin-sourced activities."""
    db = get_db_sync()
    try:
        activity = db.get(Activity, activity_id)
        if not activity or activity.source != 'garmin':
            return
        athlete = db.get(Athlete, activity.athlete_id)
        if not athlete or not athlete.strava_access_token:
            return

        # Find matching Strava activity by time window (±5 min)
        strava_name = fetch_strava_activity_name(
            athlete.strava_access_token,
            activity.start_time,
            tolerance_minutes=5,
        )
        if strava_name and not is_auto_generated(strava_name):
            activity.name = strava_name
            activity.title_source = 'strava_override'
            db.commit()
    finally:
        db.close()
```

**Scheduling:** Run `check_strava_title_update` for each Garmin-sourced
activity:
- Once immediately after Garmin ingestion (athlete may have already
  edited Strava)
- Once 24 hours later (catches delayed edits)
- On any Strava webhook that updates an activity in the same time window

#### The training log that writes itself

With the identity model applied, the activity list for a week looks
different depending on the athlete:

**Michael (Garmin-sourced, system sentence leads):**
```
Mon:  5 mi easy
Tue:  6 mi building from 8:12 to 7:15
Wed:  7 mi easy
Thu:  4 mi easy with 5 strides
Fri:  rest
Sat:  Father son, state age records 10 mile!
      └ 10 mi at 6:45
Sun:  13 mi long run at 7:30
```

**BHL (Strava-sourced, authored titles lead):**
```
Mon:  Morning Run → 5 mi easy
Tue:  Disney's Intervals on Ice. 5x4:00 @ 7:30ish...
      └ 5.5 mi with 5x4min at 7:28
Wed:  Snow and slush covered but beautiful outside 🌞
      └ 5 mi easy
Thu:  (rest)
Fri:  Warmup, 6x400m hill repeats. Stearns Hill torture chamber.
      └ 5 mi with 6 hill repeats
Sat:  Long runs are always a mental struggle...
      └ 16 mi long run
Sun:  2 @8:30, 1@7:30, 1@7:10, 2 easy
      └ 6 mi building from 8:30 to 7:10
```

BHL's authored titles carry emotion, location, weather, injury context
the system can never know. The system sentence underneath confirms the
structure. When BHL writes "6x400m hill repeats" and the system
independently says "5 mi with 6 hill repeats," trust is earned — the
system proved it understood without being told.

**No platform does this.** This is the screenshot that gets shared —
not one run, but a whole week where human voice and machine
...
**GATED — Do not build yet.** Preserve this as the next-layer coaching
contract after the sentence layer is trustworthy.

Build gate:
- same gate as Part 5: coverage >= 80% across all three athletes
- shape sentence extraction must be stable enough that gray-zone logic is
  not being layered on top of noisy / over-segmented phases
- coverage-fix work in
  `docs/BUILDER_INSTRUCTIONS_COVERAGE_FIXES.md` must ship first
...
4. **Surface in API.** Activity list and detail endpoints include
   `shape_sentence`. Return both `name` and `shape_sentence` cleanly.
5. **Surface in frontend.** Show `shape_sentence` as sentence-level
   athlete value without introducing the identity-model/title-authorship
   system yet.

**Gate note:** Steps involving `title_authorship`, `title_source`, and
the identity model are deferred until Parts 5 and 6 are unlocked. The
immediate Phase 2 work is sentence generation, suppression, and
...
Items added during founder session (post-codex review):

| ID | Addition | Spec section |
|----|---------|--------------|
| F1 | Activity identity model: 3-tier title authorship detection | Part 5 (Activity Identity Model) |
| F2 | Strava title fallback for Garmin-sourced athletes (async) | Part 5 (Strava title fallback) |
| F3 | Ambiguous titles default to authored (asymmetric cost) | Part 5 (Tier 3) |
| F4 | Wiring into all 5 coaching surfaces (yesterday's insight, home briefing, coach paragraph, activity list, detail page) | Part 7 |
| F5 | Shape data currently not consumed by any coaching surface | Part 7 (problem statement) |
