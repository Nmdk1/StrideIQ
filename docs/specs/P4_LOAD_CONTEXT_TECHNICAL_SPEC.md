...
| **H2** | L30 window | **30 athlete-local calendar days** ending **`reference_date`** (inclusive on the calendar). **DB query:** half-open UTC instant range `[local_midnight(ref−29), local_midnight(day_after_ref))` via `local_day_bounds_utc` + `get_canonical_run_activities(..., end_time_exclusive=True)` — not inclusive-end microsecond subtraction (Postgres/timestamptz-safe). |
| **H3** | **L30_max_easy_long_mi** | `max(distance_m)` over activities satisfying H1 in H2 (stored in meters, converted to miles for internal use); if none, **`None`** (caller uses tier-only baseline). |
