...
| 6 | Subscription/Tier/Payment Productionization | Complete | Stripe MVP delivered: hosted Checkout + Portal + webhook-driven subscription mirror + idempotency. ~~Originally 4-tier (Free/One-time/Guided/Premium), then converged to Free vs Pro.~~ **Reset to 2-tier (2026-03-19):** Free (30-day auto-trial) + StrideIQ Subscriber ($24.99/mo or $199/yr). Coach gated by subscription. |
...
| 11 | Coach LLM Upgrade (Multi-Model Tiering) | **Complete** | GPT-4o-mini default with Opus for high-stakes/VIP routing. Training pace tool added. RPI terminology enforced (replaced trademarked term). See `docs/LLM_MODEL_RESEARCH.md`. |
| 12 | Race QR Activation & Trial Flow | **Complete** | Full end-to-end: admin creates codes, generates QR PNG, athletes scan → register with extended trial. See `docs/RACE_QR_FIRST_7_DAYS.md`. |
...
- **2026-01-24 (Phase 6 - complete)**: Completed Phase 6 monetization + entitlement productionization: fixed Stripe subscription cancellation mirroring under newer API versions, added a self-serve **7-day trial** (plus admin grant/revoke) and exposed trial/subscription mirror in Admin for support. Converged UI/ops semantics to Free vs Pro and deprecated legacy one-time plan checkout routes. *(Note: subsequently reset to 2-tier model — Free + StrideIQ Subscriber — in March 2026.)*
...
- **2026-01-31 (Phase 11 - partial + Phase 12 complete)**: Coach LLM hardening: fixed training pace tool to use authoritative RPI calculator (not efficiency metrics), added `get_training_paces` tool with explicit system prompt guidance, auto-backfill RPI from PBs on startup. **Race QR Activation**: full end-to-end feature delivered—admin creates promo codes, generates QR PNG with auth, athletes scan → see "Race Day Special" banner → register with extended trial (30 days default), usage tracked. Added strategic `docs/RACE_QR_FIRST_7_DAYS.md` defining the Day 0–7 experience spine. Pricing updated ($14.99/mo, $149/yr). ~365,500 total lines of code. Commits: 77f4bd1, e73fc6e, 2ce59d2, c74056a, e156849, 87f46b7.
- **2026-01-31 (Bug fix - This Week cards)**: Fixed trust-breaking issue where Analytics page "This Week" cards showed only planned data (no actual distances, no checkmarks, not clickable). **Root cause**: Analytics used a separate data source (`/v1/training-plans/current/week`) that returned only planned workouts, while Home used `/v1/home` which includes actual completion data. **Fix**: (1) Backend extended `WeekDay` model with `activity_id`, `workout_id`, and `planned_distance_mi` fields; (2) Analytics page now uses `useHomeData()` (same as Home) instead of separate `useCurrentWeek()` hook; (3) Both pages render identically—completed days show checkmarks + actual distance, cards link to activity detail or calendar. Commits: bf04da8..7596d73.
- **2026-01-31 (Phase 11 - complete)**: Coach LLM Upgrade completed. **RPI terminology enforcement**: replaced trademarked term "RPI" with "RPI" (Running Performance Index) throughout all user-facing Coach outputs—system prompts (3 locations with explicit "NEVER say RPI" instructions), tool descriptions, tool response payloads (dual keys for backward compatibility), and evidence strings. Renamed frontend component `VDOTCalculator` → `TrainingPaceCalculator`. Fixed 16 test failures (model tiering tests updated to expect gpt-4o-mini default, coach routing tests updated for plain English metrics, Stripe test skipped without env). All 1326 tests pass. Commits: 3155fdb, 4a9ae35, b37ba3f.
