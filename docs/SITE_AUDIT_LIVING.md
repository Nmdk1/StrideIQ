...
**Last updated:** April 15, 2026 (Project reorg — models/, services/ package splits; god file decomposition — ai_coach.py into coaching/ package, coach_tools.py into coach_tools/ package; 53 test failures root-caused and fixed; RPI pace table regeneration; duplicate scanner distance check; Garmin adapter source contract. Prior: Plan Engine V2 wired to production, RPI table fix, Garmin Activity Files webhook, LLM-coached weekly digest, tappable briefing→coach flow, fact extraction→lifecycle wire, briefing specificity fix, Universal Kimi K2.5 routing, Limiter Phase 5, cross-training phases 1-5, Manual V2, nav consolidation)
