...
**Last updated:** April 5, 2026 (Tappable briefing→coach flow with finding_id deep link, fact extraction→lifecycle wire. Prior: Briefing specificity fix, seasonal comparison discipline, fingerprint unit-aware formatting, Universal Kimi K2.5 routing, model audit trail, Celery beat startup dispatch, AutoDiscovery savepoint fix, coach prompt quality fix, Limiter Phase 5, cross-training phases 1-5, Runtoon fix, route cleanup, page merges, wellness surfaces, Manual V2, nav consolidation, repo cleanup)
