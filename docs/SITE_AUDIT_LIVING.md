...
**Last updated:** April 21, 2026 (Unit-bypass fix on three reported surfaces (Efficiency Trend chart, Age-Graded Trajectory chart, Coach "Your data" panel) plus Recovery-day fallback honesty fix on home page; underlying API-contract debt named but deferred. Prior: Activity-page rebuild Phases 1-4 — CanvasV2 chromeless hero replaces RunShapeCanvas for run activities, Mapbox GL real-3D terrain with three-layer route + NavigationControl + desktop fullscreen + distance hover card, StreamsStack reordered HR/pace/elevation with Tukey-fence outlier clip; activity tabs consolidated 6→3 — Splits/Coach/Compare, Coach absorbs RunIntelligence + FindingsCards + WhyThisRun + GoingInCard + AnalysisTabPanel + activity.narrative, Compare placeholder remains visible as urgent-reminder pending canvas-vocabulary redesign; unskippable FeedbackModal + ReflectPill in page chrome with auto-open via useFeedbackTrigger and edit access via useFeedbackCompletion; ShareButton + ShareDrawer hosting RuntoonCard, RuntoonSharePrompt removed from app/layout.tsx (sharing is now a pull action); 39 pre-existing backend test failures resolved — Garmin source-contract via adapt_activity_detail_envelope, Garmin D5 stream-ingest refactored to _ingest_activity_detail_item_full returning Activity tuple, demo-guard mock-athlete fixes, Phase 3B narrative-LLM patch, nutrition test dates anchored to today via _d() helper, scripts hygiene docstring fix; CanvasV2 + wiki currency captured in cursor rules. Prior: Activity Intelligence + home briefing honesty fixes; demo athlete cloning pipeline + Garmin OAuth is_demo guards; nutrition Phases 1-3; morning_voice quality gates; Compare tab redesign decision captured + sequenced behind Run Shape Canvas redesign; project reorg + god-file decomposition; 53 test failures root-caused; Plan Engine V2 wired to production)
