...
**Last updated:** April 2026 — **Canonical units migration (SHIPPED):** Every API endpoint now ships distances in meters, paces in seconds-per-km, durations in seconds, elevations in meters. Frontend `useUnits()` hook converts at render time based on `Athlete.preferred_units` (country-aware default from IANA timezone, or explicit setting). LLM narrative text uses `CoachUnits` helper. CSV exports respect preferred units. The imperial-baked API field names (`distance_mi`, `pace_per_mile`, etc.) identified in the Apr 21 audit are fully eliminated. **Running vs other separation:** Home week strip + `WeekProgress` use running-only primary distance (`completed_m`, `distance_m` — meters); `other_sport_summary` rendered under trajectory; calendar API exposes `running_*` / `other_*` plus legacy all-sport `total_*`; planned-run completion uses running distance only; analytics week strip shares `WeekChipDay` with home. **Doc alignment:** §6 key-page table (run detail = CanvasV2 + 3-tab surface; `/coach` = universal Kimi via `COACH_CANARY_MODEL` default `kimi-k2.6` with Sonnet-only fallback; `/nutrition` = shipped product per `docs/wiki/nutrition.md`); §10 + §15 billing/Stripe text → two **canonical** tiers (`free`, `subscriber` in `apps/api/core/tier_utils.py`). **Prior:** Activity-page Phases 1–4 (CanvasV2 hero, Mapbox 3D, 6→3 tabs, feedback + share pull-model); backend-green sweep; wiki currency rule.
...
- **Unit-Bypass Fix on Three Reported Surfaces + Recovery-Day Honesty (Apr 21, 2026)**: Dejan Kadunc reported (support email, 2026-04-21) that the Efficiency Trend chart, Age-Graded Trajectory chart, and Coach tab "Your data" panel were rendering miles to a km-preference athlete, and that the home page kept claiming "Recovery day." every day independent of any actual recovery signal. **(1) Charts:** `apps/web/components/dashboard/EfficiencyChart.tsx` and `apps/web/components/dashboard/AgeGradedChart.tsx` both consume `EfficiencyTrendPoint.pace_per_mile` (decimal min/mile from the API) and rendered the tooltip pace with a hardcoded `/mi` suffix. Both now import `useUnits()` and route pace through the global `formatPace(secondsPerKm)` formatter via a small `minPerMileToSecPerKm` converter. **(2) Coach page:** `apps/web/app/coach/page.tsx` had zero unit-context awareness for the entire page. Added `useUnits()` and a `formatWeeklyVolume(mi)` helper that round-trips through meters into the global `formatDistance(meters)`. Both render sites — the chat empty-state brief composer (`'**This week:** ' + ... + 'mi'`) and the right-side "Your data" panel (`{...current_week_mi.toFixed(1)}mi`) — now go through the helper. The `coachBrief` `useMemo` deps include `distanceUnitShort` so the brief recomputes on toggle. **(3) Recovery-day fallback:** `apps/web/app/home/page.tsx:928` rendered the literal string `'Recovery day.'` whenever an athlete had any data but no planned workout for today — fired every day for any athlete without a plan, independent of TSB / readiness / recovery score. Same class of failure as the Jim narration incident: confidently asserting something the system has no basis for. Replaced with `'No workout planned today.'` (silent honesty). **(4) Canonical units migration — SHIPPED (Apr 2026):** The imperial-baked API field names (`distance_mi`, `pace_per_mile`, `completed_mi`, `planned_mi`) that were identified as architectural debt in the Apr 21 audit have been **fully eliminated**. Every API endpoint now ships distances in meters (`distance_m`, `completed_m`, `planned_m`, `total_distance_m`), paces in seconds-per-km (`pace_s_per_km`, `avg_pace_s_per_km`), durations in seconds (`duration_s`, `total_duration_s`), and elevations in meters. The frontend `useUnits()` hook converts at render time based on `Athlete.preferred_units` (country-aware default from IANA timezone, or explicit setting). LLM narrative text receives unit-aware context via `CoachUnits` helper (`distance_text`, `pace`, `elevation_text`). CSV exports respect preferred units. No surface in the app shows the wrong unit system. **(5) Tests:** new `apps/web/__tests__/analytics-charts-units.test.tsx` directly exercises the `pace_per_mile -> sec_per_km -> formatPace` pipeline under both unit modes (8.5 min/mi → 8:30/mi imperial / 5:17/km metric; 7.0 min/mi → 7:00/mi / 4:21/km). New `apps/web/__tests__/unit-bypass-contract.test.ts` is a source-grep regression net (same shape as `marketing-claim-contracts.test.ts`) that asserts the two chart components import `useUnits()`, the coach page imports `useUnits()` and never appends a literal `'mi'` to `current_week_mi.toFixed(...)`, and the home page never re-introduces the `'Recovery day.'` fallback. Both tests fail before the fix, pass after. Files: `apps/web/components/dashboard/EfficiencyChart.tsx`, `apps/web/components/dashboard/AgeGradedChart.tsx`, `apps/web/app/coach/page.tsx`, `apps/web/app/home/page.tsx`, `apps/web/__tests__/analytics-charts-units.test.tsx`, `apps/web/__tests__/unit-bypass-contract.test.ts`.

- **Activity Page Rebuild — Phases 1-4 (Apr 19, 2026)**: Run-activity page rebuilt end-to-end in one session, deployed to production, smoke-tested green. **(1) Phase 1 — CanvasV2 as the hero (`apps/web/components/canvas-v2/CanvasV2.tsx`, `apps/web/app/activities/[id]/page.tsx`):** new `chromeless` prop suppresses internal title/subtitle/help block; `CanvasHelpButton` moved to a minimal right-aligned slot. Run activities render `CanvasV2` chromeless as the hero, replacing the legacy `RunShapeCanvas` (preserved on disk). `TerrainMap3D.tsx` now uses real Mapbox GL with `pitch: 62`, `bearing: -20`, DEM exaggeration `3.0` set in the constructor and re-applied at `style.load` via `map.jumpTo()`. `mapbox-gl/dist/mapbox-gl.css` lifted from dynamic `useEffect` import to a static top-level import (was the cause of the production "completely dark map" regression). Built-in `hillshade` layer left untouched after `setPaintProperty` attempts threw "cannot read properties of undefined (reading 'value')" — visibility comes from terrain exaggeration plus a **three-layer route** (white casing + emerald glow + deep emerald line). `NavigationControl` mounted (rotate/tilt/zoom). Desktop-only fullscreen toggle (Google-Maps-style). Initial render zoom tightened so the course fills the frame. Distance moved from inline label to the **leftmost moment-readout hover card** (two-decimal miles + secondary time line). `StreamsStack.tsx` chart order locked to **HR top, pace middle, elevation bottom**; pace `robustDomain` switched from percentile clipping to **Tukey's fence (IQR, k=3.0)** to preserve real pace variation while clipping spikes; elevation now uses the smoothed series the splits tab uses (less pointy). Caddy CSP updated to allow Mapbox tile/style/sprite domains in `connect-src` and `blob:` in `worker-src`/`child-src`; CSP changes required a Caddy container restart, not just `caddy reload`, due to a Docker bind-mount caching artefact on Linux. **(2) Phase 2 — Tab restructure 6→3:** `Splits` (no map; the hero already has it), `Coach` (absorbs `RunIntelligence`, `FindingsCards`, `WhyThisRun`, `GoingInCard`, `AnalysisTabPanel`, and `activity.narrative`), `Compare` (placeholder; redesign sequenced behind the canvas — see `docs/specs/COMPARE_REDESIGN.md`). The `RuntoonCard` is no longer rendered at the bottom of the page (lives in the ShareDrawer now). **(3) Phase 3 — Unskippable FeedbackModal + ReflectPill (`apps/web/components/activities/feedback/`):** three sections (reflection text, RPE, workout-type confirmation) with **no escape hatch** — no X, Cancel, Skip, or backdrop-click dismissal. Save & Close stays disabled until all three are complete. Auto-classified workout types require explicit "Looks right" confirmation: `workoutTypeAcked` is only pre-true when `existingWorkoutType.is_user_override === true`. `useFeedbackTrigger` auto-opens the modal once per recent, incomplete run, gated on a `localStorage` flag so it doesn't keep popping up after save. Edits remain available later via the `ReflectPill` in the page chrome (which sources status from `useFeedbackCompletion`). Test fixes: `apiClient` mocks moved from `require()` calls to module-level `jest.fn()` instances wrapped by `jest.mock()` (the project's ESLint config doesn't register `@typescript-eslint/no-require-imports`, so the eslint-disable comments produced "rule definition not found" errors that broke the Next build); test regexes for the "no escape hatch" assertion tightened (anchored `^cancel$`, `^×$|^x$`) so they stop matching the legitimate "Save & Close" button. **(4) Phase 4 — Share is a pull action (`apps/web/components/activities/share/`, `apps/web/app/layout.tsx`):** new `ShareButton` page-chrome pill (next to `ReflectPill`, run-only) opens a new `ShareDrawer` modal that hosts the `RuntoonCard` and a roadmap placeholder for future share styles (photo overlays, customizable stats, modern backgrounds, flyovers). Drawer dismisses via close button, Escape, or backdrop click. `RuntoonSharePrompt` removed from `app/layout.tsx` (was polling `/v1/runtoon/pending` every 10s and sliding up on every recent run — explicitly rejected as push-style sharing). Component file preserved on disk for reference / rollback; intentionally not imported. Static regression test `apps/web/__tests__/layout-no-runtoon-prompt.test.ts` enforces this. **(5) Production smoke (post-deploy):** `/ping`, `/v1/home`, `/v1/coach/brief`, `/v1/activities` all 200; CanvasV2 renders Mapbox terrain end-to-end on real activity URLs; `RuntoonSharePrompt` confirmed absent from layout.

- **Backend-Green Sweep — 39 Pre-Existing Failures Resolved (Apr 19, 2026)**: The `backend-test` CI job runs only on `schedule` / `workflow_dispatch` (not on `push`), so a cluster of failures had been hiding on `main` for weeks. Running it manually after the activity-page push surfaced 39 broken tests across five clusters; all fixed in the same merge. **(1) `test_scripts_hygiene`:** `apps/api/scripts/clone_athlete_to_demo.py` docstring referenced hardcoded example emails; replaced with env-var placeholders. **(2) Garmin D5/D6 source-contract tests:** `apps/api/tasks/garmin_webhook_tasks.py` accessed `activityId` from the raw payload, violating the contract that all raw Garmin field names must be handled by `garmin_adapter.py`. Routed through `adapt_activity_detail_envelope` to extract `garmin_activity_id`. **(3) Garmin D5 stream-ingestion tests (`StopIteration`, `assert 1==2`):** `process_garmin_activity_detail_task` was re-querying for the `Activity` after `_ingest_activity_detail_item`, blowing past the mock's `side_effect` list. Refactored: `_ingest_activity_detail_item_full(...) -> (Activity | None, bool)` is the new internal API, with `_ingest_activity_detail_item` retained as a boolean wrapper for backward compat. Caller now consumes the returned `Activity` directly. **(4) Garmin feature-flag and D7 backfill tests:** the `_make_*` helpers built `MagicMock` athletes with truthy `is_demo`, which tripped the demo-account guard added later in `routers/garmin.py` and produced 403s with a plain-string detail instead of the expected JSON envelope. Helpers now explicitly set `is_demo = False`. The D7 callback test also patches `routers.garmin.is_feature_enabled` to `True` so the backfill task `delay()` is reached. **(5) `test_phase3b_graduation::test_quality_score_attached_to_result`:** the test was hitting the real LLM route and returning `suppressed=True` in CI (no API keys). Patched `services.intelligence.workout_narrative_generator._call_narrative_llm` to a canned narrative so the quality scorer is exercised. **(6) Nutrition API tests (21 failures):** `routers/nutrition.py` enforces `MAX_BACKLOG_DAYS = 60` on POST/PUT; hardcoded `2024-XX-XX` dates were now older than 60 days, returning 400s that cascaded into `KeyError: 'id'` on downstream GET/DELETE. Introduced `TODAY = date.today()` + `_d(offset_days)` helper and replaced every hardcoded date in test data and URLs.

- **Wiki Currency Made Mandatory (Apr 19, 2026)**: New `.cursor/rules/wiki-currency.mdc` makes wiki currency a binding rule with the same standard as tests — every commit that changes system behavior, surfaces, contracts, models, deploy posture, env vars, or routes must update the relevant `docs/wiki/` page in the same commit (or a follow-up commit in the same session). The rule includes a page-ownership map, a session-end check, and clear separation between wiki (operational mental model of live system), specs (forward-looking), and vision docs (what we want). Founder Operating Contract gains rule 13 (wiki must always be current) and adds `docs/wiki/index.md` to the read order. `docs/wiki/index.md` `Last updated:` bumped to April 19, 2026, and the Maintenance Contract section reworded with teeth.
...
| `/nutrition` | AI nutrition — photo/barcode/NL logging, history (60-day backfill), saved meals, fueling shelf, planning; see `docs/wiki/nutrition.md` | Working — Phases 1–3 Apr 18 |
...
- **Monetization v1 closure (Feb 26, 2026)** — legacy multi-tier purchase and entitlement surfaces shipped end-to-end at the time. **Superseded (Mar 19, 2026)** by **two-tier** canonical model — see §10 and `apps/api/core/tier_utils.py`.
...
| Item | Value |
|---|---|
| Model | **Two canonical tiers** in code: `free` \| `subscriber` (`apps/api/core/tier_utils.py`). Legacy Stripe/DB labels (`guided`, `premium`, `pro`, `elite`, `subscription`, …) **normalize to `subscriber`**. Product pricing: **$24.99/mo or $199/yr** subscriber + 30-day trial (see `docs/wiki/monetization.md`). The historical **4-tier** storefront was superseded by the two-tier reset (Mar 19, 2026). |
...
1. **Home:** `WeekProgress.completed_m` / per-day `WeekDay.distance_m` are runs only (meters); `other_activities` + `other_sport_summary` carry cross-training explicitly (summary line on the page when non-empty). Frontend converts via `useUnits()`.
2. **Calendar API:** `total_distance_m` / `total_duration_s` remain all-sport (legacy); **`running_distance_m` / `running_duration_s`** and **`other_*`** are the split fields; `get_day_status` compares planned run targets to **run** distance only; week `completed_m` uses **running** day totals (meters). Frontend calendar month/week summaries use **running** meters for the primary distance stat (fallback `total_*` for older APIs).
...
- Stripe: hosted checkout + portal + webhook entitlements for **two-tier** canonical model (`free` / `subscriber`; legacy labels map per `tier_utils.py`)
