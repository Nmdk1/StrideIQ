...
**Last updated:** April 6, 2026 (Garmin Activity Files webhook + FIT file pipeline for strength exercise sets, LLM-coached weekly digest email, dead exercise set fetch code removed. Prior: Tappable briefing→coach flow with finding_id deep link, fact extraction→lifecycle wire, briefing specificity fix, seasonal comparison discipline, fingerprint unit-aware formatting, Universal Kimi K2.5 routing, model audit trail, Celery beat startup dispatch, AutoDiscovery savepoint fix, coach prompt quality fix, Limiter Phase 5, cross-training phases 1-5, Runtoon fix, route cleanup, page merges, wellness surfaces, Manual V2, nav consolidation, repo cleanup)
