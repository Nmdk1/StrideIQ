...
**Last updated by:** Builder session — Correlation engine quality: partial correlation confounder control + direction validation
...
- **Correlation Engine Quality Fix (Mar 3, 2026)**: Correlation engine now applies partial correlation for confounder control and direction validation. New `compute_partial_correlation()` implements standard r_xy.z formula. Explicit `CONFOUNDER_MAP` dict defines which variable to partial out for each (input, output) pair — 14 entries covering motivation/enjoyment/confidence/TSB/sleep/soreness/RPE vs efficiency/pace. `DIRECTION_EXPECTATIONS` dict defines expected physiological direction for 13 input→output pairs. 5 new fields on `CorrelationFinding` model: `partial_correlation_coefficient`, `confounder_variable`, `is_confounded`, `direction_expected`, `direction_counterintuitive` (migration `correlation_quality_001`). Confounded findings (partial r drops below 0.3 threshold after controlling for confounder) set `is_active = False` — automatically excluded from Progress page and surfacing. Counterintuitive direction alone does NOT suppress (only counterintuitive + confounded = suppressed). Empirical result: founder's two problematic findings (motivation→efficiency, TSB→efficiency) survive partial correlation with ATL (partial_r = -0.57 and -0.35 respectively), meaning ATL is not the sole confounder — findings remain active but now flagged as `direction_counterintuitive = True` for review. 14 new tests (all passing). No frontend changes needed — the Progress page reads `is_active = True` findings only.
