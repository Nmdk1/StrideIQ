...
Coach Runtime V2 is protected by a shipped safety shell and now has its first visible packet-response slice. `services/coaching/runtime_v2.py` owns the fail-closed DB flag checks for `coach.runtime_v2.shadow` and `coach.runtime_v2.visible`, the runtime mode resolver (`off`, `shadow`, `visible`, `fallback`), consistency checks for runtime metadata, and the redacted `coach_runtime_v2_request` structured log event. `services/coaching/runtime_v2_packet.py` owns deterministic same-turn extraction, the Artifact 5 mode classifier, and packet assembly for the first V2.0-a slice. `query_kimi_v2_packet()` in `services/coaching/_llm.py` is the V2 coach-response call: Kimi receives the assembled packet only and no tool definitions. `alembic/versions/coach_runtime_v2_001_seed_flags.py` seeds both flags disabled with `rollout_percentage=0` and `allowed_athlete_ids=[]`. With flags off, chat is pure V1. Shadow mode still serves V1 and does not make a second coach-response LLM call. Visible mode attempts the V2 packet path; packet invariant errors, deterministic guardrail failures, empty V2 responses, V2 timeouts, provider errors, consent-disabled, no-LLM-configured, deterministic short-circuit, and budget-exceeded paths fall back to V1 with explicit runtime metadata. The umbrella log now records packet latency, V2 LLM latency when available, mode telemetry, and deterministic-check status. The first packet slice uses a temporary deterministic `legacy_context_bridge` from `_build_athlete_state_for_opus()` until native Tier 1 modules are cut over; tools are still not exposed to the V2 LLM.
...
- **Coach Runtime V2 packet slice** (`services/coaching/runtime_v2_packet.py`, `services/coaching/_llm.py`, `services/coaching/core.py`) — visible-eligible turns now assemble `coach_runtime_v2.packet.v1`, classify the Artifact 5 mode with deterministic rules, extract same-turn athlete-stated overrides without an LLM, run Kimi through `query_kimi_v2_packet()` with no tool definitions, and then apply the deterministic-only output guardrails before serving V2. Successful turns persist `runtime_mode=visible` / `runtime_version=v2`; packet, timeout, empty-response, provider, and guardrail failures demote to V1 fallback. Tests in `tests/test_coach_runtime_v2_shell.py` cover visible success, packet assembly fallback, timeout fallback, guardrail fallback, empty-response fallback, mode precedence, packet shape, real packet/LLM telemetry, and redacted runtime logs.
...
- **Coach Runtime V2 first packet path** (Apr 26, 2026): Added deterministic packet assembly, same-turn extraction, Artifact 5 mode classification, a no-tools Kimi packet response call, deterministic-only V2 output guardrails, timeout-specific fallback, and packet/LLM telemetry for visible-eligible founder testing. The path still uses a legacy context bridge while Tier 1 native state modules are built, but the LLM receives only the assembled packet and current message.
