...
Coach Runtime V2 is now protected by a shipped safety shell, not yet the V2 coaching brain. `services/coaching/runtime_v2.py` owns the fail-closed DB flag checks for `coach.runtime_v2.shadow` and `coach.runtime_v2.visible`, the runtime mode resolver (`off`, `shadow`, `visible`, `fallback`), consistency checks for runtime metadata, and the redacted `coach_runtime_v2_request` structured log event. `alembic/versions/coach_runtime_v2_001_seed_flags.py` seeds both flags disabled with `rollout_percentage=0` and `allowed_athlete_ids=[]`. With flags off, chat is pure V1. Shadow mode still serves V1 and does not make a second coach-response LLM call. If the visible flag is enabled before the V2 packet path exists, the shell records `runtime_mode=fallback`, `runtime_version=v1`, and `fallback_reason=packet_assembly_error` while serving the existing V1 answer. Existing system exits such as consent-disabled, no-LLM-configured, deterministic short-circuit, and budget-exceeded also demote visible-eligible requests to fallback metadata rather than claiming V2 served the turn.
