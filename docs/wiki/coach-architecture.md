...
- **Real Coach Standard eval framework** (`services/coaching/_eval.py`) — Phase 8 evaluator for coaching usefulness above contract shape. `tests/fixtures/coach_eval_cases.json` stores 33 structured scenarios across daily training, workout execution, nutrition, recovery, between-plan maintenance, race planning, race day, post-run interpretation, correction/dispute, emotional/frustrated athlete, and injury/pain triage. Each case carries expected coaching truths, required retrieved evidence, bad coaching patterns, `must_not` claims, a baseline answer, and severity. `tests/test_coach_real_standard.py` runs deterministic Tier 2 truth/evidence checks; `tests/test_coach_value_scoring.py` covers the Tier 3 judge payload and per-domain scoring scaffold.
