...
- **Coach Runtime V2 packet slice** (`services/coaching/runtime_v2_packet.py`, `services/coaching/_llm.py`, `services/coaching/core.py`) — visible-eligible turns now assemble `coach_runtime_v2.packet.v1`, classify the Artifact 5 mode with deterministic rules, extract same-turn athlete-stated overrides without an LLM, include an authoritative `calendar_context` block, and run Kimi through `query_kimi_v2_packet()` with no tool definitions. Successful turns persist `runtime_mode=visible` / `runtime_version=v2`; packet, timeout, empty-response, and provider failures demote to V1 fallback. V2 does not run V1's post-hoc conversation-contract/output validators on successful packet responses; quality behavior is enforced by packet structure plus Artifact 7 replay cases. Tests in `tests/test_coach_runtime_v2_shell.py` cover visible success, packet assembly fallback, timeout fallback, empty-response fallback, mode precedence, calendar context shape, temporal bridge quieting, packet telemetry, and redacted runtime logs.
