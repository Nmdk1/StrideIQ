...
- `apps/api/services/coaching/core.py` still has a stale `self.gemini_client` availability gate even though Gemini is retired from the coach chat path.
- `athlete_state` and `finding_id` context are built in `core.py`, but `query_kimi_coach()` does not currently inject `athlete_state` into the Kimi request payload.
...
- Durable facts come from `AthleteFact` via async extraction in `apps/api/tasks/fact_extraction_task.py`; this does not help the immediate next turn unless high-signal context is also handled synchronously.
...
- Remove stale Gemini availability gate from `AICoach.chat()`.
- Gate on the actual Kimi/Sonnet path.
...
- Distinguish "logged so far today" from "total daily intake."
...
- Nutrition entries may be partial. A snack is not a day total. The tool must represent coverage honestly.
...
- Some memory may need synchronous high-signal extraction for correction/context turns.
...
- Current plan week.
- Race history and invalid anchors.
- Injury context.
...
- Course matching by name/date is brittle if activity titles are inconsistent.
- PersonalBest history may not include all races if race detection/import is incomplete.
