...
**Spec:** [`docs/specs/WORKOUT_FLUENCY_REGISTRY_SPEC.md`](../../../../docs/specs/WORKOUT_FLUENCY_REGISTRY_SPEC.md) (v0.2.18)
...
| [`intervals_pilot_v1.md`](intervals_pilot_v1.md) | Pilot 4 — intervals / VO2 (`interval`, `vo2max` aliases) | **9 variants `sme_status: draft`** — await founder row-by-row approval |
...
- Pilots **1–2** fully **`approved`**. Pilot **3**: **four** core rows **`approved`**; **two** neuromuscular-touch rows (**`hills`**, **`strides`**) **`draft`** until founder sign-off (`easy_pilot_v1.md` rollup). Pilot **4** (**intervals** / VO2): **nine** rows **`draft`** until founder sign-off (`intervals_pilot_v1.md` rollup). **`repetitions`** and other **deferred** tracks per build sequence. **Phase 3 wiring** waits on **all v1-scoped** pilots per `WORKOUT_FLUENCY_BUILD_SEQUENCE.md`. Runtime wiring still follows spec §2 (P0 gate).
