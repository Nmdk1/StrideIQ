...
def upgrade() -> None:
    ...
    # Fresh-install safety: multiple historical branches reference `feature_flag`.
    # Ensure table + core columns exist before we attempt alter/drop operations below.
    ...
    # If an older schema created `feature_flag` with fewer columns, add what we need.
    op.execute(
        """
    ...
    op.alter_column('feature_flag', 'updated_at',
               ...
               nullable=False,
               existing_server_default=sa.text('now()'))
    # Fresh-install safety: constraint/index names vary across historical schemas.
    op.execute("ALTER TABLE feature_flag DROP CONSTRAINT IF EXISTS feature_flag_key_key;")
    op.execute("DROP INDEX IF EXISTS ix_feature_flag_key;")
    if op.f('ix_feature_flag_key') not in _index_names('feature_flag'):
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    # Downgrade is best-effort; this migration may run against partially created schemas.
    try:
        idxs = _index_names('feature_flag')
    except Exception:
        idxs = set()
    if op.f('ix_feature_flag_key') in idxs:
        op.drop_index(op.f('ix_feature_flag_key'), table_name='feature_flag')
    # Recreate legacy non-unique index only if missing.
    if 'ix_feature_flag_key' not in idxs:
        op.create_index('ix_feature_flag_key', 'feature_flag', ['key'], unique=False)
    op.execute("ALTER TABLE feature_flag ADD CONSTRAINT feature_flag_key_key UNIQUE (key);")
    op.alter_column('feature_flag', 'updated_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=True,
               existing_server_default=sa.text('now()'))
