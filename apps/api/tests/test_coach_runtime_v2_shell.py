...
def test_v2_flag_helper_fails_closed_on_exception(monkeypatch):
    ...
    assert is_coach_runtime_v2_enabled(COACH_RUNTIME_V2_SHADOW_FLAG, uuid4(), MagicMock()) is False
...
def test_v2_flag_helper_requires_explicit_true(monkeypatch):
    ...
    assert is_coach_runtime_v2_enabled(COACH_RUNTIME_V2_VISIBLE_FLAG, uuid4(), MagicMock()) is False
...
def test_runtime_resolver_uses_visible_over_shadow(monkeypatch):
    ...
    monkeypatch.setattr(runtime_v2, "FeatureFlagService", lambda db: _FlagService(flags))
...
def test_runtime_metadata_consistency_contract():
    assert_runtime_metadata_consistent({"runtime_mode": RUNTIME_MODE_VISIBLE, "runtime_version": RUNTIME_VERSION_V2})
    assert_runtime_metadata_consistent({"runtime_mode": RUNTIME_MODE_FALLBACK, "runtime_version": RUNTIME_VERSION_V1})

    with pytest.raises(ValueError):
        assert_runtime_metadata_consistent({"runtime_mode": RUNTIME_MODE_FALLBACK, "runtime_version": RUNTIME_VERSION_V2})
...
@pytest.mark.asyncio
async def test_visible_mode_falls_back_to_v1_until_v2_packet_path_exists(monkeypatch):
    import services.consent as consent_module
    ...
    assert kwargs["runtime_metadata"] == {
        "runtime_version": RUNTIME_VERSION_V1,
        "runtime_mode": RUNTIME_MODE_FALLBACK,
        "fallback_reason": "packet_assembly_error",
    }
...
async def test_visible_profile_short_circuit_persists_fallback_metadata(monkeypatch):
    ...
    monkeypatch.setattr(consent_module, "has_ai_consent", lambda athlete_id, db: True)
    monkeypatch.setattr(coach_core, "resolve_coach_runtime_v2_state", lambda athlete_id, db: _visible_state())
...
async def test_v2_eligible_request_emits_redacted_umbrella_log(monkeypatch):
    ...
    assert kwargs["state"].runtime_mode == RUNTIME_MODE_FALLBACK
    assert kwargs["state"].runtime_version == RUNTIME_VERSION_V1
    assert kwargs["state"].shadow_enabled is True
    assert kwargs["state"].visible_enabled is True
    assert kwargs["state"].fallback_reason == "packet_assembly_error"
...
def test_v2_seed_migration_creates_required_flags_safely():
    migration = Path(__file__).parents[1] / "alembic" / "versions" / "coach_runtime_v2_001_seed_flags.py"
