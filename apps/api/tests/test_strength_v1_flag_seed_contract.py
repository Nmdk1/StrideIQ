...
def test_alembic_head_guard_knows_about_strength_v1_001():
    text = _read(HEADS_GUARD_PATH)
    assert re.search(
        r'EXPECTED_HEADS\s*=\s*\{\s*"strength_v1_001"\s*\}', text
    ), (
        "ci_alembic_heads_check.py must declare strength_v1_001 as the "
        "expected head once this migration lands."
    )
