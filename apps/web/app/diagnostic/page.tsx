 ...
 * 
 * Comprehensive, plain-language diagnostic report for athletes:
        ...
        <div className="max-w-4xl mx-auto px-4">
                ...
                {report.executive_summary.key_findings.map((finding, idx) => (
                  <div 
                    ...
                    className={`flex items-start gap-3 p-3 rounded-lg ${
                      finding.type === 'positive' ? 'bg-green-900/20 border border-green-800/30' :
                      finding.type === 'warning' ? 'bg-orange-900/20 border border-orange-800/30' :
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between text-sm mb-2">
                  ...
                  <span className={report.volume_trajectory.current_vs_peak_pct < 0 ? 'text-orange-400' : 'text-green-400'}>
                    {report.volume_trajectory.current_vs_peak_pct > 0 ? '+' : ''}{report.volume_trajectory.current_vs_peak_pct.toFixed(0)}%
                  </span>
                </div>
                <Progress 
          ...
          <Card className="bg-slate-800 border-slate-700 mb-6">
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                {report.efficiency_analysis.runs_with_hr} runs with heart rate data
              </CardDescription>
            </CardHeader>
            <CardContent>
              {report.efficiency_analysis.trend_pct !== null ? (
                <>
                  ...
                  {/* Recent runs */}
                  {report.efficiency_analysis.recent_runs.length > 0 && (
                    <div className="mt-4">
                      <h4 className="text-sm font-medium text-slate-300 mb-2">Recent Runs</h4>
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {report.efficiency_analysis.recent_runs.slice(-5).map((run, idx) => (
                          <div key={idx} className="flex items-center justify-between text-sm py-2 bg-slate-900/30 rounded px-3">
                            <div className="flex items-center gap-2">
                              <span className="text-slate-400">{run.date}</span>
                              <span className="text-slate-300 truncate max-w-[120px]">{run.distance_km.toFixed(1)} km</span>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className="text-slate-400">HR {run.avg_hr}</span>
                              <span className="text-orange-400 font-mono">{run.efficiency.toFixed(3)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          <Card className="bg-slate-800 border-slate-700 mb-6">
            ...
            <CardContent>
              ...
              {report.data_quality.unanswerable_questions.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-slate-300 mb-2">Questions We Cannot Answer Yet</h4>
                  <ul className="space-y-1">
                    {report.data_quality.unanswerable_questions.map((q, idx) => (
          ...
          <Card className="bg-gradient-to-br from-slate-800 to-orange-900/20 border-orange-700/30 mb-6">
            ...
            <CardContent>
              ...
              {report.recommendations.medium_priority.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-blue-400 mb-2">Medium Priority</h4>
                  <div className="space-y-2">
                    {report.recommendations.medium_priority.map((rec, idx) => (
                      <div key={idx} className="bg-blue-900/10 border border-blue-800/30 rounded-lg p-3">
                        <div className="font-medium text-white mb-1">{rec.action}</div>
                        <div className="text-sm text-slate-400">{rec.reason}</div>
                      </div>
                    ))}
              ...
              {/* Do NOT Do */}
              {report.recommendations.do_not_do.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {report.recommendations.do_not_do.map((rec, idx) => (
                      <div key={idx} className="bg-red-900/10 border border-red-800/30 rounded-lg p-3">
                        <div className="font-medium text-white mb-1">{rec.action}</div>
                        <div className="text-sm text-slate-400">{rec.reason}</div>
                      </div>
                    ))}
