    ...
    <div className={`bg-slate-800 rounded-lg border border-slate-700 p-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {activity.average_heartrate && (
          <div>
            <p className="text-sm text-slate-400">Avg HR</p>
            <p className="text-lg font-semibold">{activity.average_heartrate} bpm</p>
          </div>
        )}
        {activity.max_hr && (
          <div>
            <p className="text-sm text-slate-400">Max HR</p>
            <p className="text-lg font-semibold">{activity.max_hr} bpm</p>
          </div>
        )}
        {activity.total_elevation_gain && (
          <div>
            <p className="text-sm text-slate-400">Elevation</p>
        ...
        {activity.performance_percentage && (
          <div>
            <p className="text-sm text-slate-400">Age-Graded</p>
            <p className="text-lg font-semibold">
